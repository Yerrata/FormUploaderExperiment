...
| `visa_type` | `applicant_visatype` | Transform | Copy the Candidate label into the plan and select that exact label on the live page |
| `visa_valid_until` | `applicant_visavalidtill` | Transform | Format `DD/MM/YYYY` |
| `arrived_from_country` | `applicant_arrivedfromcountry` | Transform | Copy the Candidate label and attempt one exact live option match |
...
| `next_destination_scope` | `applicant_next_dest_country_flag_r` | Transform | Map known choices to `I` or `O` and attempt the live radio operation |
...
These portal branches are not fully represented by the current Candidate:
...
- the outside-India next-destination branch beyond its `O` scope selection;
...
The 20 purpose-of-visit choices are mapped to known portal codes. An unknown value is copied into the plan and allowed to fail at the live control instead of blocking plan creation.
...
The India reference address, state, district and PIN code may come from one local `data/property.json` on the Filing Worker. Guests do not type or alter the resort's own address. When present, these values are copied into the self-contained fill plan; the executor never reloads or revalidates the file. Missing configuration leaves those operations absent rather than blocking the run.
...
- a fresh plan records the photograph currently present in the case folder;
...
## Best-effort fill-plan preparation

`formc_app/fill_plan.py` copies available Candidate, photograph and property values into an atomic per-case `fill-plan.json`. It does not use the redacted portal catalogue, launch a browser or submit a form.

The plan builder:

- copies every available mapped value into an ordered operation;
- formats dates and known closed-choice codes when possible, otherwise carries the original value;
- derives stay duration when both dates can be interpreted;
- includes the current photograph and optional property values directly in the plan;
- excludes room metadata, the legacy Form B reference and both live submission controls;
- does not reject missing fields, unknown business values, conditional branches or catalogue mismatches.

## Browser fill-only executor

`formc_app/portal_fill.py` loads only the sealed plan. It checks the plan structure, rejects submission controls and confirms that an authenticated official Form C page is open. It then performs each text fill, option selection, radio check or photograph upload directly against the live page. Dependent city/district options are attempted after they appear.

The executor has no submit operation and rejects `tmpsbmt` and `pmsbmt`. When a live operation cannot be performed it stops with the exact field/control error; otherwise it leaves the filled page open for staff review and government-portal validation.

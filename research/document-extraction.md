...
Vision APIs can read unfamiliar visa layouts and return schema-constrained JSON, but schema conformance does not make field values correct and they expose no calibrated per-field OCR confidence. OpenAI supports image input and Structured Outputs; API billing is separate from ChatGPT subscriptions and the current model table lists no free rate-limit tier for GPT-5.6 models. Cost is token-based, including image tokens, and end-to-end latency is workload-dependent, so both must be benchmarked per document. API data is not used for training by default, but abuse-monitoring logs may retain customer content for up to 30 days unless approved controls apply. ([vision](https://developers.openai.com/api/docs/guides/images-vision), [Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs), [pricing and limits](https://developers.openai.com/api/docs/models/compare), [separate billing](https://help.openai.com/en/articles/8156019-how-can-i-move-my-chatgpt-subscription-to-the-api), [data controls](https://developers.openai.com/api/docs/guides/your-data))
